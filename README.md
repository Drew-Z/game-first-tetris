# Game First Tetris

一个基于 Godot 4 的俄罗斯方块原型项目，当前同时保留两条线：
- 稳定经典主线
- Rogue 原型实验线

当前工作分支是 `feature/mobile-touch-controls-next`，当前只继续推进移动端触控深化；`main` 继续代表更稳定的经典主线。上一条 Rogue 原型节点已经保留在 `feature/rogue-prototype`，跨平台布局打磨节点已经保留在 `feature/cross-platform-layout-polish`，真实设备尺寸人工回归节点已经保留在 `feature/device-size-manual-review`，触屏输入准备节点已经保留在 `feature/touch-input-prep`。

## 当前状态

当前版本已经达到“可试玩原型版”状态，主要包含：
- 经典模式完整可玩闭环
- Rogue 模式三轮固定触发选择原型
- 最小局间承接壳层
- 暂停 / 中途重开 / 返回主菜单
- 最小声音反馈与消行闪烁
- HUD 收敛、Help 入口、响应式兜底
- 多端最小适配第一轮断点与截图回归

## 运行项目

```powershell
D:\Development\Godot\godot.cmd --path D:\workspace4Codex\game-first-tetris
```

无头验证：
```powershell
D:\Development\Godot\Godot_v4.6.1-stable_win64_console.exe --headless --path D:\workspace4Codex\game-first-tetris --quit
```

## 当前控制方式

- `← / →`：左右移动
- `↑`：旋转
- `↓`：软降
- `Space`：Hard Drop
- `C`：Hold
- `Esc`：暂停 / 关闭 Help

## 当前输入结构约定

- 当前输入层已经先按“动作类型”拆开：
  - 瞬时动作：旋转、Hard Drop、Hold、暂停 / 返回
  - 持续状态：左移、右移、软降
- 当前键盘输入会先映射到动作层，再驱动游戏行为
- 当前已经补上最小触屏输入桥接层：
  - 场景里固定提供 `TouchInputBridge` 节点
  - 触屏来源可通过 `tap_action(...)` 触发瞬时动作
  - 触屏来源可通过 `set_touch_action_pressed(...)` 驱动持续状态
  - 如需收口触屏持续状态，可调用 `release_all_touch_actions()`
- 当前已经补上极简触屏控件占位层：
  - 场景里固定提供 `TouchControls` 控件层
  - 当前只在紧凑布局里显示，用来证明“屏幕控件 -> 桥接层 -> 动作层”已贯通
  - 当前已接入：左移、右移、软降、旋转、Hard Drop、Hold、暂停
  - 当前显隐约定：
    - 只在紧凑布局里显示
    - 打开 Help、暂停、Rogue 选择、游戏结束时必须隐藏
    - 进入这些阻断状态时，会主动释放触屏持续输入，避免残留
- 当前已经把占位层收成第一版正式触屏控件原型：
  - 左移 / 软降 / 右移作为主控制按钮
  - 旋转 / 硬降 / 暂存 / 暂停作为次级动作按钮
  - 紧凑布局下会使用更明确的按钮样式、字号和按住 / 点击层级
  - 当前移动端触控深化第一步已补上：
    - 主操作区把软降作为中间主按钮，左右移动退为两侧辅助按钮
    - `393 x 852` 与 `360 x 640` 下的次级动作区统一拆成“主动作行 + 工具按钮行”
    - `旋转 / 硬降` 继续作为主要次级动作保留更明确的点击面
    - `暂存 / 暂停` 退成更轻的工具按钮，减少与主动作区和阅读区的竞争
  - 目标是先形成“真实可玩”的第一版移动端控制原型，而不是一次做完整触屏方案
  - 当前更贴近真实使用的复验结论：
    - `393 x 852`：可用；主移动按钮已基本顺手，但次级动作区仍偏紧
    - `360 x 640`：勉强可用；主按钮能用，但次级动作区和底部阅读区仍容易互相挤压
  - 当前 `360 x 640` 的定点修复方式：
    - 主移动按钮继续保留较明确的按住高度
    - 次级动作按钮的高度、字号、区块间距和面板底部留白再缩一档
    - 优先减少触屏控件总高度，降低与底部阅读区的冲突
  - 当前 `360 x 640` 的次级动作区再小收一次：
    - 继续只压次级动作区，不动主按钮按住手感
    - 次级按钮高度、字号、内容边距和面板底部留白再缩一档
    - 目标是进一步降低误触和底部阅读冲突
  - 当前 `360 x 640` 的次级动作区微调方式：
    - 旋转 / 硬降继续作为主要次级动作，保留更宽的点击面
    - 暂存 / 暂停退成独立的工具按钮行，不再和前两项共享同一横排
    - 工具按钮行右对齐显示，继续作为更轻的工具层级存在
    - 超窄态底部外边距再缩一档，继续给底部阅读区让空间
  - 当前 `360 x 640` 的再次交互复验结论：
    - 主按钮按住和状态切换恢复已基本稳定
    - 但次级动作区仍偏紧，因此当前结论继续保持“勉强可用”
    - 如果当前分支准备收口，建议把它作为当前触屏原型的最低可运行线保留，而不是强行上调为“可用”
- 后续如果接入触屏输入，优先复用这两个接入口：
  - `trigger_game_action(...)`
  - `set_game_action_pressed(...)`
- 键盘和触屏来源会分别进入动作层，再汇总成当前生效状态
- 这样做的目标是：未来接触屏输入时，不需要继续直接把逻辑堆进 `_unhandled_input()`

## 模式说明

### 经典模式
- 保持标准俄罗斯方块主循环
- 不包含 Rogue 强化与局间带入

### Rogue 模式
当前是低侵入原型，已落地：
- 开局前第一次 `3 选 1`
- 局内第二次、第三次固定触发选择
- 三种最小强化：
  - Hard Drop 额外得分
  - 消行额外得分
  - 一次出生保护
- 最小局间带入：上一局最后一次获得的强化，可作为下一局额外起始奖励

## 多端最小适配约定

### 当前断点
- 主菜单：
  - `>= 480px`：舒适区
  - `360px - 479px`：可用区，依赖滚动
  - `< 360px`：暂不保证
- 游戏界面：
  - `>= 920px`：棋盘 + HUD 左右布局
  - `< 920px`：棋盘 + HUD 上下堆叠
- HUD / Help：
  - `< 920px`：进入更紧凑排列
  - 长说明进入 Help，不回主 HUD
  - Rogue 信息区在紧凑态下压缩为“摘要层 + 效果层”

## 固定尺寸截图回归清单

目标尺寸：
- Web 小窗口：`360 x 640`
- 安卓竖屏：`393 x 852`
- 安卓竖屏：`412 x 915`
- Windows：`960 x 640`
- Windows：`1024 x 768`
- Windows：`1280 x 720`

每个尺寸固定检查：
- 主菜单
- 游戏中经典模式
- 游戏中 Rogue 模式
- Help 面板

回归产物保留约定：
- 截图回归脚本和文档结论保留在仓库里
- `artifacts/` 下的运行日志、单次截图、拼图图像默认只作为本地回归产物，不进入版本库
- 如果后续确实需要沉淀一版正式截图基线，再单独决定是否挑选少量代表性图片纳入仓库

## 当前截图回归结论

- 主菜单：全部目标尺寸下可用
- 经典模式：6 个目标尺寸下均可用
- Rogue 模式：6 个目标尺寸下均可用
- Help 面板：6 个目标尺寸下都已达到最低可用线，打开后首屏可见标题、正文和关闭方式

## 当前分支阶段结论

- `feature/cross-platform-layout-polish` 已作为“跨平台布局打磨节点”保留。
- `feature/device-size-manual-review` 已作为“真实设备尺寸人工回归节点”保留。
- 当前 `feature/touch-input-prep` 已经达到“触屏输入准备节点”可收口状态。
- 当前更适合作为这个节点保留的内容：
  - 输入结构拆分
  - 触屏桥接层
  - 第一版正式触屏控件原型
  - 触屏控件显隐与持续输入清理约定
  - `393 x 852` 的可用结果
  - `360 x 640` 的最低可运行线结论
- 当前 `feature/mobile-touch-controls-next` 只继续负责：
  - 在 `feature/touch-input-prep` 的基础上深化移动端触控方案
  - 不继续扩 Rogue 功能
  - 不修改经典 / Rogue 核心规则

## 当前人工回归摘要

- Web 小窗口（`360 x 640`）：
  - 主菜单、经典模式、Rogue 模式、Help 当前都可用
  - 但整体仍偏紧，主要依赖超窄紧凑态
- 安卓竖屏（`393 x 852`、`412 x 915`）：
  - 主菜单、经典模式、Rogue 模式、Help 当前都可用
  - `393 x 852` 的舒适度仍略低于 `412 x 915`
- Windows 常见窗口（`960 x 640`、`1024 x 768`、`1280 x 720`）：
  - 主菜单、经典模式、Rogue 模式当前都可用
  - Help 在 `960 x 640` 下已提升到可用，首屏可直接看到正文与关闭方式
  - Help 在 `1024 x 768` 和 `1280 x 720` 下可用

## 当前分支阶段结论

- `feature/device-size-manual-review` 当前已经达到“真实设备尺寸人工回归节点”可收口状态。
- 当前更适合作为节点保留的内容：
  - 三类真实场景的一轮人工回归结论
  - `960 x 640` Help 舒适度定点修复结果
- 如果先收口，下一条更推荐的主线是先做分支 / PR / 合并策略整理，再决定是否进入触屏输入准备或多端正式适配继续深化。

## 窄宽度下 Rogue 信息表达

- 摘要层：
  - `已选：...`
  - `本局带入：...`
  - `下局带入：...`
  - `下一选：第 N 轮 / X 行后`
- 效果层：
  - `硬降：+N`
  - `消行：+N/行`
  - `保护：N`

## 安卓竖屏当前收敛方式

- 重点目标尺寸：
  - `393 x 852`
  - `412 x 915`
- 当前进入安卓竖屏紧凑态后，会同时做这些收敛：
  - 棋盘缩一档显示
  - Playfield 上下留白缩小
  - Next / Hold 预览缩一档
  - Help 改用更短的竖屏帮助文案
  - Rogue 信息区继续保持“摘要层 + 效果层”
- 其中 `393 x 852` 会再进入一档更小的棋盘尺寸，优先保证 HUD 和 Help 回到首屏可读区

## 小尺寸字号 / 间距当前约定

- 重点尺寸：
  - `360 x 640`
  - `393 x 852`
- 当前策略：
  - `360 x 640`：标题字号和区块间距再收一档，但保留更明确的按钮点击高度
  - `393 x 852`：比超窄态保留更舒展一档的正文和区块留白，优先保证扫读顺畅
- 当前目标：
  - 小尺寸下看起来更顺
  - 正文字号仍可读
  - Help / 会话按钮 / Rogue 选择按钮保持可点
- 当前复验结论：
  - `360 x 640`：经典模式、Rogue 模式、Help 继续保持可用，且比上一轮更稳
  - `393 x 852`：经典模式、Rogue 模式、Help 继续保持可用
  - `412 x 915`：顺带复验未发现回退

## 极窄尺寸当前收敛方式

- 重点目标尺寸：
  - `360 x 640`
- 当前进入超窄紧凑态后，会继续做这些收敛：
  - 棋盘再缩一档
  - Playfield 最小高度再压低
  - 主菜单改用更短文案
  - 主菜单和 Help / 会话按钮提升最小点击高度
  - HUD 改用更小的间距和预览尺寸
  - Help 使用更短的首屏帮助文案
- 当前目标已经达到：
  - 主菜单可访问
  - 经典模式可用
  - Rogue 模式可用
  - Help 首屏仍可理解

## 文档入口

- `docs/task-board.md`：当前任务与阶段状态
- `docs/roadmap.md`：路线图与下一阶段建议
- `docs/test-checklist.md`：当前验证清单
